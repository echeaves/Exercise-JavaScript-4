...
function updateLocalTime() {
  ...
  let localSeconds = d.getSeconds();
  
  let localAmPm = localHours >= 12 ? 'PM' : 'AM';
  let local12Hours = localHours % 12 || 12;
  
  let localTimeStr = `${local12Hours}:${localMinutes}:${localSeconds}  ${localAmPm}`;
...
localButton.addEventListener('click', updateLocalTime);